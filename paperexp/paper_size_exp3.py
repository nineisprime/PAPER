...
Code for generating Figure 12 (Right) in the manuscript
...
ntrials = 20
edge_ratio_ls = np.array([0.1, 0.2, 0.4, 0.6, 0.8, 1])
print(edge_ratio_ls)
...
for j in range(len(edge_ratio_ls)):
    for i in range(len(n_ls)):    
        for it in range(ntrials):
            ...
            edge_ratio = np.sqrt(n) * edge_ratio_ls[j]
            ...
            with open("pickles/paper_exp3.pkl", 'wb') as f:
